...
Sistema de consulta documental con RAG agéntico: indexa manuales de procesos (CRM y manejo de leads inmobiliarios) y responde preguntas **citando el archivo y la página exacta** de donde salió cada afirmación. Todo corre local en una RTX 5060 Ti (16 GB VRAM) — sin nube, sin API keys.

Construido sobre **LlamaIndex Workflows** (orquestación agéntica event-driven) + **Ollama** (inferencia local). Expuesto como API REST (FastAPI) y como servidor **MCP**, de modo que otros agentes (incluido opencode) pueden consultar tus documentos directamente.
...
- **Citas verificables**: el prompt le dice al LLM que cite con `[n]`, pero los números los asigna *el nodo realmente recuperado*, no el modelo. El post-proceso reemplaza cada `[n]` con los metadatos reales (`file_name`, `page_label`) y su score de similitud. El LLM **no puede inventar** una fuente porque las fuentes se adjuntan desde los `source_nodes` reales.
...
| Componente | Rol |
|---|---|
| `ingest.py` | Lee los PDFs de `data/`, los chunkiza con ventana de 3 oraciones, genera embeddings bge-m3 y persiste el índice en `storage/`. Idempotente: si el índice existe, no reindexa (salvo `--force`). |
...
| `api.py` | FastAPI: `GET /health` y `POST /query` (json con `question` y `rerank`). Devuelve `answer` + `sources[]` (ref, file, page, score, text). |
...
| `storage/` | Índice persistido (vector store + docstore). Se regenera con `ingest.py --force`. |
...
- **Paquetes Python** (ver `requirements.txt`): `llama-index-core`, `llama-index-llms-ollama`, `llama-index-embeddings-ollama`, `llama-index-readers-file`, `fastapi`, `uvicorn`, `mcp[cli]`, `fastmcp`, `pydantic`. Para la evaluación: `ragas==0.4.3`, `datasets`, `langchain`, `langchain-core>=1.4.7`, `langchain-community==0.4.2`, `langchain-google-vertexai` + un shim (ver sección 6, trampa 7).
...
Respuesta: `{"answer": "... [1][2]", "sources": [{"ref": 1, "file": "...pdf", "page": "7", "score": 0.63, "text": "..."}]}`. Documentación interactiva en `http://127.0.0.1:8000/docs`.

### 4.5 MCP (para opencode y otros agentes)
...
Tras reiniciar opencode, cualquier agente puede invocar la tool `query_documents_tool(question, rerank)` — es decir, **opencode consulta tus documentos citando página y archivo**.
